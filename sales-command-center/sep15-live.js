  ...
  function apply(){
    ...
    s={
      ...
      tasks:[
        ...
        {id:'sep16-3',name:'Fred Genovese',action:'Positive Response Carryover — Convert to Appointment',note:'Positive response and worked on 9/15. No hard appointment has been recorded yet.',priority:'hot',done:false},
        {id:'sep16-4',name:'Paul Creason',action:'Yesterday 7:30 PM Appointment — Outcome Needed',note:'No outcome has been reported yet. Keep his current pipeline status until the result is confirmed.',priority:'high',done:false},
        {id:'sep16-5',name:'Joy Tennyson',action:'Missed Appointment Recovery',note:'Missed the 9/15 9:30 AM appointment. Offer two concrete reschedule options.',priority:'high',done:false},
        {id:'sep16-6',name:'Matthew Barber',action:'Friday 10:00 AM Appointment — Keep Warm',note:'Appointment is set for Friday 9/18 at 10:00 AM. Client email is attached to the calendar invite.',priority:'high',done:false},
        {id:'sep16-7',name:'Fresh Lead Flow',action:'KEEP NEW LEADS RUNNING',note:'Use the gaps around today\'s two verified appointments to keep fresh opportunities entering the pipeline.',priority:'high',done:false}
      ]
    };
    ...
    if(heroCopy)heroCopy.textContent='Current board: 1 appointment set today, 0 pitches, 0 sales, and $0 Written AV. Stephanie is at 10:30 AM, Aurora at 5:30 PM, and Mary Peterson is now secured for Thursday at 6:30 PM.';
    ...
    if(head){
      ...
      focus.innerHTML='<div class="eyebrow">WEDNESDAY GAME PLAN • TWO CLOSE WINDOWS</div><div class="leadFlowTitle">Stephanie 10:30 • Aurora 5:30 • Mary secured for Thursday</div><div class="leadFlowCopy">Protect today\'s two verified appointments, keep Mary warm for Thursday at 6:30 PM, convert Fred while he is still warm, and keep fresh lead flow moving during the open blocks.</div><div class="leadFlowMix"><div><b>20%</b><span>Carryover Recovery</span></div><div><b>35%</b><span>Fresh Lead Flow</span></div><div><b>45%</b><span>Appointments + Close</span></div></div>';
      head.parentNode.insertBefore(focus,head);
      const schedule=document.createElement('section');schedule.className='card daySchedule';
      schedule.innerHTML='<div class="eyebrow">CLOSE WINDOWS</div><div class="dayScheduleTitle">Verified client schedule</div><div class="dayScheduleItems"><span class="dayScheduleItem"><strong>10:30 AM</strong> Stephanie Blackmon — Appointment</span><span class="dayScheduleItem"><strong>5:30 PM</strong> Aurora Kramer — Appointment</span><span class="dayScheduleItem"><strong>Thu 6:30 PM</strong> Mary Peterson — Appointment</span><span class="dayScheduleItem"><strong>Fri 10:00 AM</strong> Matthew Barber — Appointment</span><span class="dayScheduleItem"><strong>Sat 11:30 AM</strong> Victoria Thompson — Appointment</span></div>';
    ...
    const pipelineCounts=[41,25,5,13,10,5,24];
