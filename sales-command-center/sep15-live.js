  ...
  function apply(){
    ...
    s={
      ...
      tasks:[
        ...
        {id:'sep15-1',name:'Germain Kennard',action:'SOLD — $6,296.16 Written AV',note:'Closed today from the 1:00 PM appointment. Written AV is $6,296.16; Issued AV remains blank until the policy issues.',priority:'hot',done:true},
        {id:'sep15-2',name:'Stephanie Blackmon',action:'2:00 PM Appointment — Protect + Close',note:'Appointment moved to today at 2:00 PM and worked today. Her client email is attached to the calendar invite, which is marked orange.',priority:'hot',done:false},
        {id:'sep15-3',name:'Christine Milham',action:'Pitch Completed Today — Worked',note:'Worked today and pitch completed. No hard appointment/follow-up time is set. Keep her existing Sold / $6,638.52 production history intact.',priority:'high',done:true},
        {id:'sep15-4',name:'Paul Creason',action:'7:30 PM Appointment — Protect + Close',note:'Worked today and appointment set for 7:30 PM. Lead source: Windback JT Leads. Protect the evening close window and confirm before the call.',priority:'hot',done:false},
        {id:'sep15-5',name:'Fred Genovese',action:'Positive Response Today — Convert to Appointment',note:'Positive response and worked today. No hard appointment is set yet. Lead source: Montague New.',priority:'hot',done:false},
        {id:'sep15-6',name:'Matthew Barber',action:'Fri 10:00 AM Appointment — Protect + Close',note:'Worked today and appointment set for Friday, September 18 at 10:00 AM. Client email is attached to the calendar invite and the event is orange.',priority:'hot',done:false},
        {id:'sep15-7',name:'Due Follow-Up Queue',action:'Work the hottest overdue conversations first',note:'Recover missed appointments and pitched business before spending too much time on colder aged leads.',priority:'high',done:false},
        {id:'sep15-8',name:'Fresh Lead Flow',action:'KEEP NEW LEADS RUNNING',note:'Keep fresh traffic moving between appointment windows so the next close is already entering the funnel.',priority:'high',done:false}
      ]
    };
    ...
    if(heroCopy)heroCopy.textContent='Current board: 2 positive responses, 4 appointments set today, 2 pitches completed, 1 sale, and $6,296.16 Written AV. Fred is warm, Paul remains at 7:30 PM, and Matthew Barber is booked Friday at 10:00 AM.';
    ...
    if(head){
      ...
      focus.innerHTML='<div class="eyebrow">TUESDAY GAME PLAN • BUILD ON THE CLOSE</div><div class="leadFlowTitle">Convert Fred + protect Paul at 7:30</div><div class="leadFlowCopy">Germain already closed for $6,296.16 Written AV. Fred is a fresh positive response, Matthew is secured for Friday at 10:00 AM, and Paul remains the key close window tonight.</div><div class="leadFlowMix"><div><b>25%</b><span>Warm Lead Conversion</span></div><div><b>25%</b><span>Fresh Lead Flow</span></div><div><b>50%</b><span>Appointments + Close</span></div></div>';
      head.parentNode.insertBefore(focus,head);
      const schedule=document.createElement('section');schedule.className='card daySchedule';
      schedule.innerHTML='<div class="eyebrow">CLOSE WINDOWS</div><div class="dayScheduleTitle">Current appointment schedule</div><div class="dayScheduleItems"><span class="dayScheduleItem"><strong>9:30 AM</strong> Joy Tennyson — MISSED</span><span class="dayScheduleItem"><strong>1:00 PM</strong> Germain Kennard — SOLD $6,296.16</span><span class="dayScheduleItem"><strong>2:00 PM</strong> Stephanie Blackmon — Appointment</span><span class="dayScheduleItem"><strong>7:30 PM</strong> Paul Creason — Appointment</span><span class="dayScheduleItem"><strong>Wed 5:30 PM</strong> Aurora Kramer — Appointment</span><span class="dayScheduleItem"><strong>Fri 10:00 AM</strong> Matthew Barber — Appointment</span><span class="dayScheduleItem"><strong>Sat 11:30 AM</strong> Victoria Thompson — Appointment</span></div>';
    ...
    const pipelineCounts=[41,24,5,13,10,5,24];
