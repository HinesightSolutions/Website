(() => {
  ...
  const plan = {
    ...
    av: 5502.12,
    avGoal: 10000,
    targets: { warm: 10, pitch: 2, sale: 2 },
    ...
    tasks: [
      ...
      { id: 'sep11-12', name: 'Valerie Alexander', action: 'SOLD — $2,425.20', note: 'Valerie converted today for $2,425.20 Written AV. Keep her on the board as today’s second sale.', priority: 'hot', done: true },
      { id: 'sep11-13', name: 'Stephanie Blackmon', action: 'Fresh Positive Response — Convert Now', note: 'Fresh response today from the NextGen file. Work her while she is engaged and move toward a firm appointment or plan review.', priority: 'high', done: false },
      ...
      { id: 'sep11-8', name: 'Fresh Lead Flow', action: 'KEEP RUNNING LEADS — Do Not Let the Funnel Dry Up', note: 'You have produced 10 positive responses and two sales today. Keep fresh traffic moving while converting the people who are raising their hands.', priority: 'high', done: false },
  ...
  try {
    ...
    if (heroCopy) heroCopy.textContent = 'Current board: 10 positive responses, 1 appointment set, 2 pitches, 2 sales, $5,502.12 Written AV, and $3,076.92 Issued AV. You hit the sales goal — keep converting the fresh interest and keep lead flow active.';
    ...
    if (head) {
      ...
      leadFlow.innerHTML = `
        ...
        <div class="leadFlowTitle">Ten positive responses and two sales are on the board</div>
        <div class="leadFlowCopy">Valerie Alexander converted for $2,425.20, bringing today to $5,502.12 Written AV and two sales. Stephanie Blackmon is the newest fresh opportunity. Keep converting the remaining warm conversations without shutting off fresh lead flow.</div>
        <div class="leadFlowMix"><div><b>35%</b><span>Fresh Lead Flow</span></div><div><b>45%</b><span>Fresh / Warm Conversion</span></div><div><b>20%</b><span>Pitch + Due Follow-Up</span></div></div>`;
    ...
    patchClient('Valerie Alexander', { work:'Sep 11', appt:'—', stage:'Sold', source:'#2 Exclusive LM', av:2425.20 });
    patchClient('Stephanie Blackmon', { work:'Sep 11', appt:'—', stage:'Positive Response', source:'NextGen Leads' });
    ...
    const pipelineCounts = [39, 22, 6, 10, 9, 5, 24];
